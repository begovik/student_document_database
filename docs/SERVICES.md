...
| `discovery` (ddgs, openalex) | Пошук кандидатів, сідування `search_queries` | — (LLM-доповнення запитів: `GEMINI_API_KEY 1-3` → `harvester/discovery/querygen_llm.py`) | `docs/COMMANDS.md#1` / `harvester/discovery/querygen.py:56` |
...
| `bibliography` | Добирання з літератури каталогу | `GEMINI_API_KEY 1..3` (LLM-витяг → спільний `LLMClient`), пошук `harvester/discovery/ddgs_search.py` + `harvester/net/guards.py`; PDF лише тимчасовий | `docs/COMMANDS.md#6-витяг-літератури-та-добирання-джерел-harvester-bibliography` / `harvester/bibliography/service.py:56` |
...
Наразі реально запускаються лише `ddgs` та `openalex`. Інші назви каналів з конфігурації є зарезервованими і мають залишатися `enabled: false`, доки не з'являться адаптер, тести та інтеграція у supervisor.
